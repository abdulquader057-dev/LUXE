...
export default function SettingsPageClient({ isAdmin }: { isAdmin: boolean }) {
  ...
  const filteredMenu = SETTINGS_MENU.filter(item => item.id !== "permissions" || isAdmin);
  ...
  const renderSection = () => {
    switch (activeTab) {
      ...
      case "permissions":
        return <PermissionSettings isAdmin={isAdmin} />;
      case "support":
...
function PermissionSettings({ isAdmin }: { isAdmin: boolean }) {
  const { user } = useAuth();
